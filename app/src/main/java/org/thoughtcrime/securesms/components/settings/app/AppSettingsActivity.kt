  ...
  override fun onCreate(savedInstanceState: Bundle?, ready: Boolean) {
    ...
    SignalStore.settings.onConfigurationSettingChanged.observe(this) { key ->
      if (key == SettingsValues.THEME) {
        ...
        recreate()
      } else if (key == SettingsValues.APP_UI_MODE) {
        wasConfigurationUpdated = true
        recreate()
      } else if (key == SettingsValues.TEXT_FONT ||
        key == SettingsValues.MESSAGE_FONT_SIZE ||
        key == SettingsValues.CHAT_LIST_VIEW ||
        key == SettingsValues.SMOOTH_TRANSITIONS) {
        wasConfigurationUpdated = true
