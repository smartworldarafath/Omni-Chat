...
class PrivacySettingsFragment : DSLSettingsFragment(R.string.preferences__privacy) {
  ...
  private var pendingBiometricAction: PendingBiometricAction? = null
  private var pendingScreenLockEnabled: Boolean = false
  ...
  private fun getConfiguration(state: PrivacySettingsState): DSLConfiguration {
    return configure {
      ...
      if (state.isObsoletePasswordEnabled) {
        ...
        switchPref(
          title = DSLSettingsText.from(R.string.preferences_app_protection__biometric),
          summary = DSLSettingsText.from(R.string.preferences_app_protection__biometric_summary),
          isChecked = isKeyguardSecure && state.screenLock,
          onClick = {
            if (!isKeyguardSecure) {
              showScreenLockUnavailableSnackbar()
            } else if (biometricAuth.canAuthenticate(requireContext())) {
              pendingBiometricAction = PendingBiometricAction.SCREEN_LOCK
        ...
        switchPref(
          title = DSLSettingsText.from(R.string.preferences_app_protection__face_lock),
      ...
      switchPref(
        ...
        onClick = {
          if (!ServiceUtil.getKeyguardManager(requireContext()).isKeyguardSecure) {
            ...
            pendingBiometricAction = PendingBiometricAction.PAYMENT_LOCK
            biometricAuth.updatePromptInfo(createPaymentLockPromptInfo())
  ...
  private fun createScreenLockPromptInfo(enable: Boolean): PromptInfo {
    return PromptInfo.Builder()
      .setAllowedAuthenticators(BiometricDeviceAuthentication.ALLOWED_AUTHENTICATORS)
      .setTitle(
        requireContext().getString(
          if (enable) {
            R.string.ScreenLockSettingsFragment__use_signal_screen_lock
          } else {
            R.string.ScreenLockSettingsFragment__turn_off_signal_lock
          }
        )
  ...
  private fun showScreenLockUnavailableSnackbar() {
    Snackbar
      .make(
        requireView(),
  ...
  private fun showFaceLockUnavailableSnackbar() {
    Snackbar
      .make(
        requireView(),
        resources.getString(R.string.preferences_app_protection__face_lock_unavailable),
        Snackbar.LENGTH_LONG
      )
  ...
  private fun handleBiometricAuthenticationSuccess() {
    when (pendingBiometricAction) {
      PendingBiometricAction.PAYMENT_LOCK -> viewModel.togglePaymentLock(false)
      PendingBiometricAction.SCREEN_LOCK -> setScreenLockEnabled(pendingScreenLockEnabled)
      PendingBiometricAction.FACE_LOCK -> setScreenLockEnabled(pendingScreenLockEnabled)
      null -> Unit
    }
  ...
  private enum class PendingBiometricAction {
    PAYMENT_LOCK,
    SCREEN_LOCK,
    FACE_LOCK
  }
