...
data class Boost(
  ...
  private class SelectionViewHolder(itemView: View) : MappingViewHolder<SelectionModel>(itemView) {
    ...
    override fun bind(model: SelectionModel) {
      ...
      model.boosts.zip(boostButtons).forEach { (boost, button) ->
        ...
        applyBoostButtonStyle(button, isSelected)
      ...
      applyCustomAmountStyle(model.isCustomAmountFocused)
      custom.setOnClickListener {
        playTapAnimation(custom)
      }
    ...
    private fun applyBoostButtonStyle(button: MaterialButton, isSelected: Boolean) {
      if (isSelected) {
        val targetColor = SELECTED_GREEN
        val currentColor = button.backgroundTintList?.defaultColor ?: targetColor

        if (currentColor == targetColor) {
          button.backgroundTintList = ColorStateList.valueOf(targetColor)
        } else {
        ...
        button.strokeColor = ColorStateList.valueOf(SELECTED_GREEN_STROKE)
        button.setTextColor(SELECTED_GREEN_TEXT)
        button.elevation = 8f.dp
        button.rippleColor = ColorStateList.valueOf(SELECTED_GREEN_RIPPLE)
      } else {
        button.backgroundTintList = ContextCompat.getColorStateList(context, R.color.signal_selectable_button_background_tint)
        button.strokeColor = ContextCompat.getColorStateList(context, R.color.signal_selectable_button_stroke)
        button.setTextColor(ContextCompat.getColor(context, R.color.signal_text_primary))
        button.elevation = 0f
        button.rippleColor = ContextCompat.getColorStateList(context, R.color.signal_button_secondary_ripple)
      }
    }

    private fun applyCustomAmountStyle(isSelected: Boolean) {
      val defaultTextColor = ContextCompat.getColor(context, R.color.signal_text_primary)
      custom.setTextColor(if (isSelected) SELECTED_GREEN_TEXT else defaultTextColor)
      custom.setHintTextColor(if (isSelected) SELECTED_GREEN_TEXT else defaultTextColor)
      custom.elevation = if (isSelected) 8f.dp else 0f
    }

    private fun playTapAnimation(view: View) {
      view.animate()
        .scaleX(0.94f)
        .scaleY(0.94f)
        .setDuration(90L)
    ...
    private val Float.dp: Float
      get() = this * itemView.resources.displayMetrics.density
  }
  ...
  companion object {
    private val SELECTED_GREEN = Color.rgb(48, 252, 160)
    private val SELECTED_GREEN_STROKE = Color.rgb(28, 218, 133)
    private val SELECTED_GREEN_TEXT = Color.rgb(4, 50, 34)
    private val SELECTED_GREEN_RIPPLE = Color.rgb(166, 255, 215)
