...
private fun Ready(state: GatewaySelectorState.Ready, onEvent: (GatewaySelectorBottomSheetEvent) -> Unit) {
  val badgeEntrance = remember { Animatable(0f) }

  LaunchedEffect(Unit) {
    badgeEntrance.animateTo(
      targetValue = 1f,
  ...
  BadgeImage112(
    ...
    modifier = Modifier
      .size(112.dp)
      .graphicsLayer {
        alpha = badgeEntrance.value
        val fullScreenProgress = 1f - badgeEntrance.value
        val fullScreenScale = 1f + (fullScreenProgress * 1.85f)
        scaleX = fullScreenScale
        scaleY = fullScreenScale
        translationY = fullScreenProgress * -240f
        rotationZ = fullScreenProgress * -18f
        shadowElevation = fullScreenProgress * 28f
      }
  )
