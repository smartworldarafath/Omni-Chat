...
fun DeviceListScreen(
  ...
  val introEntrance = remember { Animatable(0f) }
  var isLinkButtonAnimating by remember { mutableStateOf(false) }
  val linkButtonScale by animateFloatAsState(
    targetValue = if (isLinkButtonAnimating) 0.94f else 1f,
    animationSpec = tween(durationMillis = 180, easing = FastOutSlowInEasing),
    label = "link-device-button-scale"
  )
  val scope = rememberCoroutineScope()

  LaunchedEffect(Unit) {
    introEntrance.animateTo(
      targetValue = 1f,
  ...
  Column(horizontalAlignment = Alignment.CenterHorizontally, modifier = modifier.verticalScroll(rememberScrollState())) {
    Icon(
      ...
      modifier = Modifier.graphicsLayer {
        alpha = introEntrance.value
        scaleX = 0.84f + (introEntrance.value * 0.16f)
        scaleY = 0.84f + (introEntrance.value * 0.16f)
        translationY = (1f - introEntrance.value) * -34f
      }
    )
    Text(
    ...
    Buttons.LargeTonal(
      onClick = {
        if (!isLinkButtonAnimating) {
          scope.launch {
            isLinkButtonAnimating = true
            delay(130)
            isLinkButtonAnimating = false
            delay(70)
            onLinkNewDeviceClicked()
          }
        }
      ...
      modifier = Modifier
        .graphicsLayer {
          scaleX = linkButtonScale
          scaleY = linkButtonScale
          shadowElevation = if (isLinkButtonAnimating) 18f else 6f
        }
    ...
    Column(modifier = Modifier.fillMaxWidth()) {
      ...
      if (state.deviceListLoading) {
        ...
        state.devices.forEachIndexed { index, device ->
          DeviceRow(device, index, state.isInternalUser, onDeviceSelectedForRemoval, onEditDevice)
        }
      }
    }
...
@Composable
fun DeviceRow(device: Device, index: Int = 0, isInternalUser: Boolean, setDeviceToRemove: (Device) -> Unit, onEditDevice: (Device) -> Unit) {
  ...
  val rowIconEntrance = remember(device.id) { Animatable(0f) }

  LaunchedEffect(device.id) {
    delay(index * 80L)
    rowIconEntrance.animateTo(
      targetValue = 1f,
  ...
  Row(
    ...
    Image(
      ...
      modifier = Modifier
        .graphicsLayer {
          alpha = rowIconEntrance.value
          scaleX = 0.7f + (rowIconEntrance.value * 0.3f)
          scaleY = 0.7f + (rowIconEntrance.value * 0.3f)
          rotationZ = (1f - rowIconEntrance.value) * -8f
        }
