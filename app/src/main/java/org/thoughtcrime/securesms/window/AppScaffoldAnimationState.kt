...
object AppScaffoldAnimationDefaults {
  ...
  val SmoothTweenEasing = CubicBezierEasing(0.17f, 0.17f, 0f, 1f)
  val InitAnimationOffset = 48.dp

  fun <T> tween() = if (CoreUiDependencies.smoothTransitionsEnabled) {
    tween<T>(durationMillis = 320, easing = SmoothTweenEasing)
  } else {
    tween<T>(durationMillis = 200, easing = TweenEasing)
  }
}
