...
public final class ConversationListItem extends ConstraintLayout implements BindableConversationListItem, Unbindable {
  ...
  private void applyChatListViewMode() {
    SettingsValues.ChatListView mode = SignalStore.settings().getChatListView();
    ...
    switch (mode) {
      ...
      default:
        ...
        avatarSize      = getResources().getDimensionPixelSize(R.dimen.conversation_list_avatar_size);
        summaryLines    = 2;
        nameSizeSp      = 16f;
        summarySizeSp   = 14f;
        break;
    ...
    setPadding(getPaddingStart(), verticalPadding, getPaddingEnd(), verticalPadding);
    subjectView.setMaxLines(summaryLines);
    subjectView.setLines(summaryLines);
    subjectView.setTextSize(summarySizeSp);
    fromView.setTextSize(nameSizeSp);

    ViewGroup.LayoutParams layoutParams = contactPhotoImage.getLayoutParams();
    if (layoutParams.width != avatarSize || layoutParams.height != avatarSize) {
      layoutParams.width = avatarSize;
      layoutParams.height = avatarSize;
      contactPhotoImage.setLayoutParams(layoutParams);
    }
  }
